$(document).ready(function () {
  ...
  function ajaxGet(page) {
    ...
    $.ajax({
      ...
      url:
        "http://localhost:8080/swispshop/api/tai-khoan/all" + "?page=" + page,
      success: function (result) {
        $.each(result.content, function (i, taiKhoan) {
          ...
          if (role == "ROLE_EMPLOYEE") {
            taiKhoanRow +=
              '<td width="0%">' +
              '<input type="hidden" id="idTaiKhoan" value=' +
              taiKhoan.id +
              ">" +
              "</td>";
            taiKhoanRow +=
              '<td><button class="btn btn-success btnUpRole">Nâng quyền Admin</button></td>';
          }
          ...
          if (role == "ROLE_MEMBER" || role == "ROLE_EMPLOYEE") {
            taiKhoanRow +=
              ...
              ">" +
              "</td>";
            if (taiKhoan.isBlocked)
              taiKhoanRow +=
                '<td><button class="btn btn-primary btnX" >Mở khóa</button></td>';
            else
              taiKhoanRow +=
                '<td><button class="btn btn-danger btnX" >Khóa</button></td>';
  ...
  function ajaxPostTaiKhoan() {
    ...
    $.ajax({
      ...
      type: "POST",
      contentType: "application/json",
      url: "http://localhost:8080/swispshop/api/tai-khoan/save",
  ...
  $(document).on("click", ".btnXoa", function () {
    ...
    if (confirmation) {
      $.ajax({
        type: "DELETE",
        url:
          "http://localhost:8080/swispshop/api/tai-khoan/delete/" + taiKhoanId,
  ...
  $(document).on("click", ".btnX", function () {
    ...
    if (confirmation) {
      $.ajax({
        type: "PUT",
        url:
          "http://localhost:8080/swispshop/api/tai-khoan/switchStatus/" +
          taiKhoanId,
        success: function (resultMsg) {
          resetData();
        },
  ...
  $(document).on("click", ".btnUpRole", function () {
    ...
    if (confirmation) {
      $.ajax({
        type: "PUT",
        url:
          "http://localhost:8080/swispshop/api/tai-khoan/upRoleToAdmin/" +
          taiKhoanId,
        success: function (resultMsg) {
          resetData();
        },
