function sendContact() {
  ...
  var flag = 0;
  if (
    !/^[^0-9][A-z0-9_]+([.][A-z0-9_]+)*[@][A-z0-9_]+([.][A-z0-9_]+)*[.][A-z]{2,4}$/.test(
      email
    )
  ) {
  ...
  $.ajax({
    ...
    url: "http://localhost:8080/swispshop/createContact",
    success: function (result) {
      alert(
        "Cảm ơn quý khách đã liên hệ với chúng tôi. Shop sẽ phản hồi sớm cho các bạn"
      );
      window.location.href = "/swispshop/contact";
    },
    error: function (e) {
      alert("Error: ", e);
      console.log("Error", e);
