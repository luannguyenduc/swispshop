$(document).ready(function () {
  ...
  function ajaxGet(page) {
    ...
    $.ajax({
      ...
      url: "http://localhost:8080/swispshop/api/san-pham/all" + "?page=" + page,
  ...
  function ajaxPostLapTop() {
    ...
    $.ajax({
      ...
      type: "POST",
      contentType: "application/json",
      url: "http://localhost:8080/swispshop/api/san-pham/save",
  ...
  $(document).on("click", ".btnCapNhatLapTop", function (event) {
    ...
    var href = "http://localhost:8080/swispshop/api/san-pham/" + sanPhamId;
  ...
  $(document).on("click", ".btnCapNhatOther", function (event) {
    ...
    $.get(href, function (sanPham) {
      ...
      $(".modal-title").text(
        "Cập nhật sản phẩm của danh mục " + sanPham.danhMuc.tenDanhMuc
      );
  ...
  $(document).on("click", ".btnXoaSanPham", function () {
    ...
    if (confirmation) {
      $.ajax({
        async: false,
        type: "DELETE",
        url: "http://localhost:8080/swispshop/api/san-pham/delete/" + sanPhamId,
  ...
  $(document).on("click", ".btnChiTiet", function () {
    ...
    var href = "http://localhost:8080/swispshop/api/san-pham/" + sanPhamId;
    $.get(href, function (sanPham) {
      $(".hinhAnh").attr("src", "/swispshop/img/" + sanPham.id + ".png");
      $(".tenSanPham").html(
      ...
      if (checkTenDanhMuc != -1) {
        $(".mauSac").html(
          "<span style='font-weight: bold'>Màu sắc: </span>" + sanPham.mauSac
        );
        $(".kichThuoc").html(
          "<span style='font-weight: bold'>Kích thước: </span>" +
            sanPham.kichThuoc
        );
        $(".thietKe").html(
      ...
      $(".soLuong").html(
        "<span style='font-weight: bold'>Số lượng trong kho: </span>" +
          sanPham.soLuong
      );
    });
