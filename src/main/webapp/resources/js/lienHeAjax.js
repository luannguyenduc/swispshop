$(document).ready(function () {
  ...
  function ajaxGet(page) {
    ...
    $.ajax({
      ...
      url: "http://localhost:8080/swispshop/api/lien-he/all" + "?page=" + page,
      success: function (result) {
        $.each(result.content, function (i, lienHe) {
          ...
          lienHeRow +=
            '<td width="0%">' +
            '<input type="hidden" id="lienHeId" value=' +
            lienHe.id +
            ">" +
            "</td>" +
  ...
  $(document).on("click", ".btnChiTiet", function (event) {
    ...
    var href = "http://localhost:8080/swispshop/api/lien-he/" + lienHeId;
    $.get(href, function (lienHe) {
      $("#noiDungLienHe").html(
        "<strong>Nội dung liên hệ</strong>: <br> " + lienHe.noiDungLienHe
      );
      $("#ngayLienHe").html(
        "<strong>Ngày liên hệ</strong>: " + lienHe.ngayLienHe
      );
      ...
      if (lienHe.trangThai == "Đã trả lời") {
        $("#noiDungTraLoi").html(
          "<strong>Nội dung trả lời</strong>: " + lienHe.noiDungTraLoi
        );
        $("#ngayTraLoi").html(
          "<strong>Ngày trả lời</strong>: " + lienHe.ngayTraLoi
        );
      } else {
        $("#noiDungTraLoi").html("");
        $("#ngayTraLoi").html("");
  ...
  function ajaxPostReply() {
    ...
    $.ajax({
      ...
      type: "POST",
      contentType: "application/json",
      url: "http://localhost:8080/swispshop/api/lien-he/reply",
